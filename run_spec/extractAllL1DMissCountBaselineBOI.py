...
benchmarks.sort()
datadir = os.getenv("LAB_PATH") + '/results-proj-baseline/X86/run_micro'
...
for bm in benchmarks: 
    for cpu in plt_gem5_cpus:
        for mem in plt_mem_ctls:
            ...
            gem5GetStat(datadir+"/"+bm+"/"+cpu+"/"+mem,'system.cpu.l1d.overall_misses::total'),
            gem5GetStat(datadir+"/"+bm+"/"+cpu+"/"+mem,'system.cpu.l1d.overall_accesses::total'),
            gem5GetStat(datadir+"/"+bm+"/"+cpu+"/"+mem,'system.cpu.l1d.overall_miss_rate::total')
            ])

df = pd.DataFrame(rows, columns=['benchmark','cpu', 'mem', 'cycles','instructions', 'Ops', 'Ticks','Host', 'avgmemaccesslatency','busutilit','bandwidthtotal','totalbuslatency',                                       'averagewritebandwidth', 'miss_count', 'total_access', 'miss_rate'])
df['ipc'] = df['instructions']/df['cycles']
df['cpi']= 1/df['ipc']
print(df)
stat = 'miss_count'
with open('miss_count_l1d_baseline.csv', 'w', newline='') as csvfile:
    header = ['benchmark']
